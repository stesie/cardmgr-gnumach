/*
 * PCMCIA Card Manager daemon for gnumach 1
 * Copyright(C) 2006 by Stefan Siegl <stesie@brokenpipe.de>
 * 
 *************************************************************************

    The Card Manager daemon for GNU/Linux is subject of the Mozilla
    Public License Version 1.1 also. The gnumach specific parts are
    subject of the GPL Version 2 only however.

    Concerning the gnumach specific parts which are copyright of myself,
    you may apply any later version of the GPL (i.e. later than 2)
...
/*
 * the {{{, }}} marks are folding marks by the way, i.e. you ought to
 * enable your emacs' folding-mode here :-)
 *
...
/*====================================================================*/
#ifdef MACH
#define socket_t hurd_socket_t /* prevent name clash */
#include <hurd.h>
#undef socket_t
#include <mach.h>
...
/*
 * on mach we cannot just call ioctl(), we got to use get_status and
 * set_status for this ...
 */
#define ioctl(a,b,c) gnumach_do_ioctl(a,b,c)
#include "mach-ioctl.c"

/* #define syslog(a,b...) do { fprintf(stderr, "cardmgr: " b);	\
 *   fprintf(stderr, "\n"); } while(0)
 */
...
#ifndef MACH
/* {{{ device open related cruft for GNU/Linux

 *
 * on gnumach we don't have not char device with a major number but we 
 * distinct devices by name. this is, our socket devices are named
 * `socket0', `socket1', etc.
 */
...
/*
 * try to open socket `sock' using mode.
 *
 * concerning mach, we need to pass a file descriptor back. In mach speak
 * we got a mach_port_t however, which is natural_t and therefore should
 * fit into an `int' by definition. Therefore, let's keep casting folks.
 *
...
#ifdef MACH
    char *device_name;
    if(asprintf(&device_name, "socket%d", sock) < 0)
       return -1;

    device_t device;
    error_t err = device_open(device_master,
			      ((mode & S_IWRITE) ? D_WRITE : 0) | D_READ,
			      device_name, &device);
    
    free(device_name);

    return err ? -1 : ((int) device);	
#else /* not MACH ... */
 ...
 *
 * we currently support pcmcia-based ethernet devices
 * on gnumach only, so far ...
 */
...
#ifdef MACH
/* 
 * for the moment this cardmgr is going to stay fucking quiet!
 */
...
#ifndef MACH
/* {{{ we don't have no modules on mach ...
...
static void done(void)
{
    ...
    for(;;) {
      ...
      for(i = 0; i < sockets; i ++) 
	if(socket[i].fd
	   && device_close((mach_port_t) socket[i].fd) == D_INVALID_OPERATION) 
	  fail ++;
	else
	  socket[i].fd = 0;
...
int main(int argc, char *argv[])
{
    ...
    if(get_privileged_ports(0, &device_master)) {
      fprintf(stderr, "cardmgr: superuser privileges required.\n");
      return -1;
    }
#else
	...
	for (i = 0; i < sockets; i++) {
	  ...
	  void *worker(void *p) {
	    ...
	    unsigned int bytes;
	    int i = (int) p;
	    int excess = one_pass ? 1 : 5;

	    while(excess --) {
	      ret = device_read((mach_port_t) socket[i].fd,
				one_pass ? D_NOWAIT : 0,
				0, 4, (char **) &buf, &bytes);

	      if(ret == D_WOULD_BLOCK)
		return NULL; /* no events to handle in one shot mode,
			      * get outta here */

	      if(ret == D_DEVICE_DOWN)
		return NULL; /* the device is shutting down, i.e. we called
			      * device_close from another thread, silently
			      * quit ... */

	      if(ret != D_SUCCESS || bytes != 4) {
		syslog(LOG_INFO, "device_read(%d) failed, stop.", i);

		if(! ret)
		  vm_deallocate(mach_task_self(), buf, bytes);

		return NULL;
	      }

	      memcpy(&event, (char *) buf, 4);
	      vm_deallocate(mach_task_self(), buf, bytes);
#else
...
#ifdef MACH
	    } /* end of worker's for(;;) loop ... */
	    return NULL;
	  } /* end of worker */

	  if(one_pass) {
	    /* don't fork threads, just call the worker once for every sock */
	    worker((void *) i);
	  }
	  else {
	    /* daemon-mode: fork a thread for every socket on first pass */
	    if(! pass) {
	      cthread_t th = cthread_fork(worker, (void *) i);
	      cthread_detach(th);
	    }
	  }
#endif
...
#ifdef MACH
	/* On MACH-systems don't fork at all, doesn't work, whyever -
	 * this ought to be further tracked: FIXME */
