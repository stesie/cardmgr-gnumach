...
static int open_sock(int sock)
{
    ...
    device_t device;
    error_t err = device_open(device_master, D_READ, device_name, &device);
...
static int do_cmd(int fd, int cmd)
{
    ...
    case C_SUSPEND:
	ret = ioctl(fd, DS_SUSPEND_CARD, NULL);
	break;
	
    case C_RESUME:
	ret = ioctl(fd, DS_RESUME_CARD, NULL);
	break;

    case C_RESET:
	ret = ioctl(fd, DS_RESET_CARD, NULL);
	break;

    case C_EJECT:
	ret = ioctl(fd, DS_EJECT_CARD, NULL);
	break;
	
    case C_INSERT:
	ret = ioctl(fd, DS_INSERT_CARD, NULL);
	break;
    }
    return ret;
}
